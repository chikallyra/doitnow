    ...
    <div class="mx-10 lg:mx-32 mt-10 pb-10">
        ...
        <div class="border p-3 rounded-lg">
            ...
            <div class="step-container m-4 flex items-center {{ $index > 0 ? 'hidden' : '' }}" data-index="{{ $index }}">
                <input type="checkbox" id="togglePopup-{{ $index }}" class="form-checkbox h-5 w-5 text-blue-600 bg-white border-gray-400" disabled>
                <label for="togglePopup-{{ $index }}" class="ml-2 text-black font-normal">{{ $step['description'] }}</label>
            </div>
            ...
            <div id="popup-{{ $index }}" class="flex justify-center items-center hidden fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 z-50">
                <div class="bg-white p-8 rounded-lg">
                    ...
                    <form id="popupForm-{{ $index }}" class="popup-form">
                        @if($step['type'] === 'file')
                        <div class="mb-6">
                            <label for="file-input-{{ $index }}" class="block mb-2 text-sm font-medium text-gray-900">{{ $step['description'] }}</label>
                            <input type="file" id="file-input-{{ $index }}" name="file" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        </div>
                        ...
                        @else
                        <div class="mb-6">
                            <label for="default-input-{{ $index }}" class="block mb-2 text-sm font-medium text-gray-900">{{ $step['description'] }}</label>
                            <input type="text" id="default-input-{{ $index }}" name="default" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        </div>
                        @endif
                        <button type="button" class="mt-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" onclick="closePopup({{ $index }})">Close</button>
                        <button type="button" class="mt-4 px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600" onclick="submitStep({{ $index }})">Submit</button>
                    </form>
                </div>
            </div>
            @endforeach
            ...
            <div id="popup-modal" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="relative bg-white rounded-lg shadow">
                        <button class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center" data-modal-hide="popup-modal">
                        ...
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <button data-modal-hide="popup-modal" type="button" class="py-2.5 px-5 ml-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
                                No, cancel
                            </button>
                        </div>
                    </div>
