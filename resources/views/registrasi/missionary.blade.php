...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-8 bg-white">
  ...
  <div class=" relative justify-center items-center mx-8 lg:mt-24">
    <h1 class="mt-6 text-[30px] text-center font-bold text-gray-900 sm:text-3xl  lg:text-4xl">
    ...
    <h2 class="text-center mt-5 text-lg lg:text-[25px] font-bold text-gray-800">Sign Up </h2>
      <div class="mb-4 ">
        <div class="max-w-md relative justify-center items-center mx-auto">
            <form class="max-w-sm mx-auto mt-6 " action="{{ route('register.user') }}" method="POST">
              @csrf
              <div class="mb-5">
                <label for="name" class="block mb-2 text-sm font-medium text-gray-900 ">Name</label>
                <input type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5" placeholder="Enter your full name " name="name" value="{{ old('name') }}" required />
                @error('name')
              ...
              <div class="mb-5">
                ...
                <input type="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5" placeholder="Enter your full email " name="email" value="{{ old('email') }}" required />
                @error('email')
                  <div class="text-red-500">
                      {{ $message }}
                  </div>
